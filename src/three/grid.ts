...
function makeAtlas(): CanvasTexture {
  ...
  if (ctx) {
    ...
    ctx.font = `48px "JetBrains Mono Variable", ui-monospace, monospace`;
    for (let i = 0; i < CHARSET.length; i += 1) {
      ctx.fillText(CHARSET[i], i * GLYPH_PX + GLYPH_PX / 2, GLYPH_PX / 2 + 2);
    }
  }
...
function layoutGlyphs(): Glyph[] {
  ...
  for (let step = 1; step <= 4; step += 1) {
    ...
    push(label, at, 0.14); // along X, tucked beside the axis line
    push(label, 0.16 + (label.length * GLYPH_W) / 2, -at + 0.07);
  }
  push('x', 2.35, 0.14);
  push('z', 0.16 + GLYPH_W / 2, -2.35);
  return glyphs;
}

function makeLabels(): InstancedMesh {
  const glyphs = layoutGlyphs();
  const geometry = new PlaneGeometry(0.06, 0.06);
